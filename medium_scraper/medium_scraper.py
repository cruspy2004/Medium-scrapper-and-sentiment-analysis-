...
def calculate_metrics(articles):
    ...
    # Total responses for the topic
    total_responses = df.shape[0]  # Number of articles
    ...
    print(f"Sentiment Counts: Positive={total_positive}, Negative={total_negative}, Neutral={total_neutral}")

    # Calculate sentiment score
    sentiment_score = (total_positive * 1) + (total_negative * -1) + (total_neutral * 0)
    ...
    # Debug print for overall sentiment and score
    ...
    return total_responses, author_metrics, total_positive, total_negative, total_neutral, overall_sentiment


def save_to_csv(articles, tag, output_dir=None):
...
def main():
    ...
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'output')
    metrics_filename = os.path.join(output_dir, f"{tag}_author_metrics.csv")
